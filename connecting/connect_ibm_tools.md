...
You can connect IBM command-line interfaces, applications, and tools to your {{site.data.keyword.dv_short}} instance. 
{: shortdesc}
...
- To create a connection with SSL, complete the following steps:
  ...
  2. In the {{site.data.keyword.dv_short}} web console, download the SSL certificate from the **Connect your applications to the database** page.
  ...
  5. Configure the Db2 client on the DataStage server.
     ...
     where `<db_alias>` is your name for the {{site.data.keyword.dv_short}} instance.
  ...
  7. Test the SSL connection in one of the following ways:
     ...
     - Test the connection by using CLP. Issue the following command to connect to the {{site.data.keyword.dv_short}} instance:

       `db2 connect to <db_alias> user <user_id>`

       where `<db_alias>` is your name for the {{site.data.keyword.dv_short}} instance and `<user_id>` is your {{site.data.keyword.dv_short}} user ID. You are prompted to enter your password.
    
     - Test the connection by using CLI. Issue the following command to connect to the {{site.data.keyword.dv_short}} instance:

       `db2cli validate -dsn <alias> -connect -user <user_id> -passwd <password>`

        where `<alias>` is an alias that you created by using the **db2cli writecfg** command, `<user_id>` is your {{site.data.keyword.dv_short}} user ID, and `<password>` is your {{site.data.keyword.dv_short}} password.

- To create a connection without SSL, catalog the target {{site.data.keyword.dv_short}} instance by completing the following steps:

  1. Catalog the target {{site.data.keyword.dv_short}} node so that client applications can connect to it. Run the following CLP commands:
  ...
  2. Catalog the remote {{site.data.keyword.dv_short}} instance so that client applications can connect to it. Run the following command:

     `db2 catalog db BLUDB as <db_alias> at node <node_name>`

     where `<db_alias>` represents your name for the {{site.data.keyword.dv_short}} instance and `<node_name>` represents your name for the node.
...
When you use {{site.data.keyword.dv_short}} as a connection target, the performance of IBM InfoSphere Data Replication partly depends on the bandwidth of the network that separates its target engine from the {{site.data.keyword.dv_short}} instance. Physical distance also affects performance: ideally, IBM InfoSphere Data Replication is as close as possible to the {{site.data.keyword.dv_short}} instance. Network topology also affects performance. For example, ideally, the IBM InfoSphere Data Replication target engine runs on a VM in the same VPN (security domain) as the target instance. The fewer the network nodes (for example, firewalls or routers) to traverse, the better. 
...
1. Choose one of the following approaches to make your connection:
   ...
   - To create a connection with SSL, complete the following steps:
     ...
     e. Catalog the {{site.data.keyword.dv_short}} node so that client applications can connect to it. Issue the following command:
     ...
     f. Catalog the remote {{site.data.keyword.dv_short}} instance so that client applications can connect to it. Issue the following command:
     ...
     - Test the connection by using CLP by issuing the following command to connect to the {{site.data.keyword.dv_short}} instance:

       `db2 connect to <db_alias> user <user_id>`

       where `<user_id>` is your {{site.data.keyword.dv_short}} user ID. You are prompted to enter your password.
                
     - Test the connection by using CLI by issuing the following command to connect to the {{site.data.keyword.dv_short}} instance:

       `db2cli validate -dsn <alias> -connect -user <user_id> -passwd <password>`

       where `<alias>` is a DSN alias that you created by using the **db2cli writecfg** command, `<user_id>` is your {{site.data.keyword.dv_short}} user ID, and `<password>` is your {{site.data.keyword.dv_short}} instance password.
        
   - To create a connection without SSL, complete the following steps:

     a. Catalog the {{site.data.keyword.dv_short}} node so that client applications can connect to it. Issue the following command:
     ...
     `<Db2_Warehouse_IP_address>` is the IP address of the {{site.data.keyword.dv_short}} server.

     `<port_number>` is the port that is used to connect to {{site.data.keyword.dv_short}} without using an SSL connection. If you are using the default port, specify `50000`.
            
     b. Catalog the remote {{site.data.keyword.dv_short}} instance so that client applications can connect to it. Issue the following command:
     ...
     - Test the connection by using CLI by issuing the following command to connect to the {{site.data.keyword.dv_short}} instance:
       ...
       where `<alias>` is a DSN alias that you created by using the **db2cli writecfg** command, `<user_id>` is your {{site.data.keyword.dv_short}} user ID, and `<password>` is your Db2 Warehouse on Cloud password.
...
A connection between your IBM® Data Server Manager and your {{site.data.keyword.dv_short}} instance enables you to monitor and manage the database from the Data Server Manager web console. 
{: shortdesc}
...
3. Click the ![+ sign inside a circle](images/icon_R_plus.gif "Add icon") icon to add a database connection. On the **Add Database Connection** page under the **Database Connection** tab, enter the required information in the following fields:
   ...
   - *Database name*: `BLUDB`
   - *Host name*: Enter the {{site.data.keyword.dv_short}} host name 
   ...
   - *User ID*: Your {{site.data.keyword.dv_short}} user ID 
   - *Password*: Your {{site.data.keyword.dv_short}} password 
...
Command line processor plus (CLPPlus) is included in the Db2 driver package. CLPPlus provides a command-line interface that you can use to connect to a {{site.data.keyword.dv_short}} instance. You can use CLPPlus to define, edit, and run statements, scripts, and commands.
{: shortdesc}
...
2. To start CLPPlus with a connection to a {{site.data.keyword.dv_short}} instance that uses the entries in the `db2dsdriver.cfg` file, run the following command:
...
4. You can discover assets from connections to {{site.data.keyword.dv_short}}, which gives you the ability to add all tables from the connection as data assets to a project. Select **Discover data assets** and choose a project.
...
These instructions explain how to create a connection from IBM® SPSS® Statistics to a {{site.data.keyword.dv_short}} instance.
{: shortdesc}
...
3. Use the ODBC Data Source Administrator window to add an ODBC data source name (DSN) for the {{site.data.keyword.dv_short}} instance:
